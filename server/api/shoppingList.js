  ...
  try {
    ...
    if (!quantity) {
      if (shoppingList.hasIngredient(ingredientToUpdate)) {
        ...
        await shoppingList.addIngredients(ingredientToUpdate, {
          through: { sliQuantity: initialAmount * 1 + 1 },
        })
      } else {
        await shoppingList.addIngredient(ingredientToUpdate, {
    ...
    if (quantity === 0) await shoppingList.removeIngredient(ingredientToUpdate)
      ...
      await shoppingList.addIngredient(ingredientToUpdate, {
        through: { sliQuantity: quantity, cost: cost },
      })
    }
...
router.post("/", authenticateToken, async (req, res, next) => {
  try {
    ...
    const currentPantry = await Pantry.findByPk(pantryId, {
      include: Ingredient,
    })
    ...
    await shoppingList.ingredients.forEach((addMeToPantry) => {
      ...
      if (currentPantry.hasIngredient(addMeToPantry)) {
        ...
        currentPantry.addIngredient(addMeToPantry, {
          through: {
            pantryQty: sliQuantity * 1 + initialAmount * 1,
            cost: cost,
          },
        })
      } else {
